...
public partial class Backpacks
{
	...
	private void yamlWindowDrawer(int id)
	{
		...
		if (GUILayout.Button(Localization.instance.Localize("$bp_config_editor_save")) && !hasErrors)
		{
			save();
			activeConfigFileData = null;
		}

		if (configSync.IsSourceOfTruth && GUILayout.Button(Localization.instance.Localize("$bp_config_editor_apply")) && !hasErrors)
		{
		...
		GUI.enabled = true;
		if (GUILayout.Button(Localization.instance.Localize("$bp_config_editor_discard")))
		{
			activeConfigFileData = null;
		}
		...
		{
			if (GUI.GetNameOfFocusedControl() == $"{ModName} yaml textarea for {file}" && Event.current.type is EventType.KeyDown or EventType.KeyUp && Event.current.isKey)
			{
			...
			if (!collapsed[file])
			{
				...
				GUI.SetNextControlName($"{ModName} yaml textarea for {file}");
